-- Two Thumbs Up - Complete Database Migration
-- Run this in Supabase SQL Editor:
-- https://supabase.com/dashboard/project/fxbxmixsszmyviypyrzj/sql/new
...
create table if not exists public.posts (
  ...
  type text not null default 'blog' check (type in ('blog', 'quote')),
  tags text[] default '{}',
...
create policy "posts_public_read_published" on public.posts for select to public
...
create policy "comments_public_read" on public.comments for select to public using (
...
create policy "comments_member_insert" on public.comments for insert to authenticated with check (
  ((select auth.uid()) = author_id)
  and exists (select 1 from public.posts where posts.id = comments.post_id and posts.status = 'published')
);

drop policy if exists "comments_self_or_admin_delete" on public.comments;
create policy "comments_self_or_admin_delete" on public.comments for delete to authenticated
  using (((select auth.uid()) = author_id) or (select public.is_admin()));

-- Reactions policies
drop policy if exists "reactions_public_read" on public.reactions;
create policy "reactions_public_read" on public.reactions for select to public using (true);

drop policy if exists "reactions_self_insert" on public.reactions;
create policy "reactions_self_insert" on public.reactions for insert to authenticated with check (
  ((select auth.uid()) = user_id)
  and exists (select 1 from public.posts where posts.id = reactions.post_id and posts.status = 'published')
);

drop policy if exists "reactions_self_delete" on public.reactions;
create policy "reactions_self_delete" on public.reactions for delete to authenticated
...
create policy "images_insert" on storage.objects for insert to authenticated with check (
  bucket_id = 'images'
  and (
    ((storage.foldername(name))[1] = 'avatars' and (storage.foldername(name))[2] = ((select auth.uid())::text))
    or ((storage.foldername(name))[1] = 'posts' and (select public.is_admin()))
  )
);
...
create policy "images_delete" on storage.objects for delete to authenticated using (
  bucket_id = 'images'
  and (
    ((storage.foldername(name))[1] = 'avatars' and (storage.foldername(name))[2] = ((select auth.uid())::text))
    or (select public.is_admin())
